...
const Staff = (props: any) => {
	...
	const [validationShape, setValidationShape] = useState({
		...
		countryCode: yup.string().min(0, "Min is 0").required("Country Code is required"),
		...
		pctOfPayoutCheck: yup
			...
			.min(0, "Min is 0")
			.max(100, "max is 100"),
		pctOfTips: yup.number().required("Is required").positive().integer().min(0, "Min is 0").max(100, "max is 20"),
		pctOfProductSales: yup
			...
			.min(0, "Min is 0")
			.max(100, "max is 20"),
		order: yup.number().required("Is required").positive().integer().min(1, "Min is 1"),
	...
	useEffect(() => {
		...
		setTempFilter(selectedMultiOptions.filter((element) => tempArr.includes(element)));
	...
	useEffect(() => {
		if (selectedMultiParent.length == 0) {
			...
			setErrorMsg("error");
		} else if (selectedMultiParent.includes("admin") && selectedMultiOptions.length == 0) {
			setSelectedMultiOptions([]);
			setErrorMsg("");
		}
	...
	const handleMultiParent = (selectedItems: any) => {
		const multiOptions: any = [];
		const multiRoleOptions: any = [];
		for (let i = 0; i < selectedItems.length; i++) {
			multiOptions.push(selectedItems[i].value);
			if (selectedItems[i].value == "admin") {
				multiRoleOptions.push("admin");
			} else if (selectedItems[i].value != "admin") {
				multiRoleOptions.push("stylist");
			}
		}
		setSelectedRole(multiRoleOptions);
		...
		selectedMultiParent.forEach((value: any) => {
			if (value) {
				const newFliterJob: any = allServices.filter((data: any) => {
					return Object.values(data).join(" ").toLocaleLowerCase().includes(value.toLocaleLowerCase());
				});
	...
	const handleSubmit = (values: any) => {
		...
		if (view) {
			...
			} else if (values.profileCategoryId.length == 0 && values.editedMultiParent.length == 0) {
			} else {
																...
																<FormGroup>
																	...
																	<Col sm="9">
																		<FormControl
																			...
																			{resultFilterCategory &&
																				resultFilterCategory.length &&
																				resultFilterCategory.map((value: any) =>
																					value.map((val: any) => {
																						return (
																							<option value={val.id}>
																								{val.name}
															...
															<React.Fragment>
																...
																<div className="row">
																	<div className="col-md-8">
																		...
																		serviceParentPercentage.length ? (
																			serviceParentPercentage.map(
																				(value: any) => {
																					return (
																						<FormGroup>
																							<FormLabel className="col-sm-3 control-label">
																							...
																							<Col sm="9">
																								<FormControl
																									...
																									style={
																										errorMsg ==
																											"error" &&
																										newObj[value.id] <= 0 ? {border: "1px solid red"} : {border: "1px solid #e5e6e7"}
																									}
																									value={
																										newObj &&
																										newObj[value.id] ? newObj[ value .id ] : ""
																									}
																									onChange={
																										handleServiceChange
																									}
																		...
																		<FormGroup>
																			...
																			<Col sm="9">
																				<FormControl
																					...
																					max="100"
																					isInvalid={
																						errors.pctOfProductSales && touched.pctOfProductSales
																					}
																				/>
																			</Col>
																		</FormGroup>
