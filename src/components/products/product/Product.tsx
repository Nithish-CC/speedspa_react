...
const Products = (props: any) => {
	...
	const dataFilter: any = () => {
		if (allProducts && allProducts.data && allProducts.data.length > 0) {
			let status: any = allProducts.data
				.map((value: any) => {
					return value.brand;
				})
				.filter((v: any, i: any, currentStatus: any) => currentStatus.indexOf(v) === i);
			setBrand(status);
			return status;
		}
	};
	...
	const filterData = () => {
		if (params.brand) {
			const newFliterJob: any = allProducts.data.filter((data: any) => {
				return Object.values(data).join(" ").toLocaleLowerCase().includes(params.brand.toLocaleLowerCase());
			});
