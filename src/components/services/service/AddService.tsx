...
const Service = (props: any) => {
		...
		if (view == true) {
			...
			if (serviceData.links) {
				...
				if (serviceData.links.addOns && serviceData.links.addOns.length > 0) {
					...
					if (serviceData.links.additional && serviceData.links.additional.length > 0) {
					...
					if (serviceData.links.onOption && serviceData.links.onOption.length > 0) {
	...
	const addAddOnService = (parentIndex: any, addonIndex: any) => {
		const filteredLinkedServices = [...linkedServices];
		filteredLinkedServices[parentIndex].addonServiceIds.splice(addonIndex + 1, 0, {
			id: null,
		});
		setLinkedServices(filteredLinkedServices);
	};
	...
	const handleAddOnChange = (e: any, parentIndex: any, addOnIndex: any) => {
		const filteredLinkedServices = [...linkedServices];
		filteredLinkedServices[parentIndex].addonServiceIds[addOnIndex].id = e.target.value;
		setLinkedServices(filteredLinkedServices);
	};
	...
	const handleActiveTime = (event: any) => {
		setActiveTime({
			...
			[event.target.eventName]: Number(event.target.value),
		});
	};
	...
	const handleAddCostChange = (event: any) => {
		if (event == "reset") {
			...
			setAddCostChangeVal({ resourceId: event.target.value, resourceName: text });
		} else {
			const { options, selectedIndex } = event.target;
