                                    ...
                                    <div className='ibox-content'>
                                            ...
                                            <div className='row'>
                                                <div className='col-md-5 col-sm-8'>
                                                    <div className='form-group'>
                                                        <label>Search</label>
                                                        <input type='text' value={params.text} name='text' placeholder='Search by service name' className='form-control' onChange={handleChange} />
                                                    </div>
                                                </div>
                                                <div className='col-md-3 col-sm-4'>
                                                    <div className='form-group'>
                                                        <label>Type</label>
                                                        <select value={params.category} className='form-control' name='category' onChange={handleChange}>
                                        ...
                                        <div className='table-responsive'>
                                            <table className='table table-bordered table-striped table-hover dataTables-example'>
                                                <thead>
                                                    <tr>
                                                        <th className={field !== 'name' ? 'sorting' : orderBy ? 'sorting_asc' : 'sorting_desc'} onClick={e => handleSortChange('name')}>Service Name</th>
                                                        <th className={field !== 'categoryName' ? 'sorting' : orderBy ? 'sorting_asc' : 'sorting_desc'} onClick={e => handleSortChange('categoryName')}>Service Type</th>
                                                        <th className={`text-center ${field !== 'price' ? 'sorting' : orderBy ? 'sorting_asc' : 'sorting_desc'}`} onClick={e => handleSortChange('price')}>Price</th>
                                                        <th className={`text-center ${field !== 'displayPrice' ? 'sorting' : orderBy ? 'sorting_asc' : 'sorting_desc'}`} onClick={e => handleSortChange('displayPrice')}>Display Price</th>
                                                        <th className='text-center'>Action</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {allServices && allServices.length ? (
                                                        allServices.slice((activePage - 1) * perPage, activePage * perPage).map((service: any, index: any) => {
                                                            return (
                                                                <tr className='gradeX' key={index}>
                                                                    <td>{service.name}</td>
                                                                    <td>{service.categoryName}</td>
                                                                    <td className='text-center'>${(service.price/100)}</td>
                                                                    <td className='text-center'>{service.displayPrice}</td>
                                                                    <td className='text-center'>
                                                                        <Link style={{'cursor': 'pointer', 'color': '#2a6954'}} key={index} to={`/services/view/${service.id}`}>
                                                                            <i title='View | Edit' className='far fa-edit'></i>
                                                                        </Link>
                                                                        &nbsp; 
                                                                        <a style={{'cursor': 'pointer', 'color': '#2a6954'}}  onClick={() => deletePopup(service, index)}>
